...
test test_vega([] {
	...
	for (double h : {.01, .001, .0001, 0.00001, .000001}) {
		...
		double dp_ = (p_ - _p) / (2 * h);
		double vega_test = (p_ - 2 * p + _p) / (h * h);
		//double a, b;
		//a = dp - dp_;
		//b = vega_ * h * h / 2;
		ensure(fabs(dp - dp_) <= vega_test * h * h / 2);
	}
	});
...
static AddIn xai_black_put_implied(
	...
	.Arg(XLL_DOUBLE, L"f", L"is the forward.", L"100")
	.Arg(XLL_DOUBLE, L"p", L"is the price", L"3.988")
	.Arg(XLL_DOUBLE, L"k", L"is the excecute price.", L"100")
	.Arg(XLL_DOUBLE, L"t", L"is the time .", L"0.25")
	.Category(L"XLL")
	.FunctionHelp(L"Return the volatility ")
);
