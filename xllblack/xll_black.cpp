...
test test_vega([]() {
	...
	for (double h : {.01, .001, .0001, 0.00001, .000001}) {
		double v0 = vega(f, sigma, k, t);
		double v1 = put(f, sigma + h, k, t);
		double v2 = put(f, sigma - h, k, t);
		double dp_ = (v1 - v2) / (2 * h);
		double gamma = (v1 - 2 * p + v2) / (h * h);
		//double a, b;
		//a = dp - dp_;
		//b = gamma * h * h / 2;
		ensure(fabs(v0 - dp_) <= gamma * h * h / 2);
	}
...
static AddIn xai_black_put_implied(
	...
	.Arg(XLL_DOUBLE, L"f", L"is the forward.", L"100")
	.Arg(XLL_DOUBLE, L"p", L"is the price of the put option.", L"4.5")
	.Arg(XLL_DOUBLE, L"k", L"is the strike.", L"100")
	.Arg(XLL_DOUBLE, L"t", L"is the time in years to expiration.", L"0.25")
	.Category(L"XLL")
	.FunctionHelp(L"Return the implied volatility for the put option")
);
