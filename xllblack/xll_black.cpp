...
test test_vega([] {
	...
	for (double h : {.01, .001, .0001, 0.00001, .000001}) {
		...
		double dp_ = (p_ - _p) / (2 * h);
		double vega_ = (p_ - 2 * p + _p) / (h * h);
		//double a, b;
		//a = dp - dp_;
		//b = vega_ * h * h / 2;
		ensure(fabs(dp - dp_) <= vega_ * h * h / 2);
	}
	});
...
static AddIn xai_black_put_implied(
	...
	.Arg(XLL_DOUBLE, L"f", L"is the forward.", L"100")
	.Arg(XLL_DOUBLE, L"p", L"is the price of the European put option.", L"3.988")
	.Arg(XLL_DOUBLE, L"k", L"is the strike.", L"100")
	.Arg(XLL_DOUBLE, L"t", L"is the time in years to expiration.", L"0.25")
	.Category(L"XLL")
	.FunctionHelp(L"Return the volatility of the European put option implied by Black model.")
);
...
test test_black_put_implied([]() {
	...
	double sigma = put_implied_volatility(100, 3.988, 100, 0.25);

	ensure(fabs(sigma - 0.2) < 0.001);
