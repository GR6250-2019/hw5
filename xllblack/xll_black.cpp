...
test test_vega([]() {
    ...
    for (double h : {.01, .001, .0001, 0.00001, .000001}) {
        ...
        double p_ = put(f, sigma, k, t);
        double _p = put(f, sigma - h, k, t);
        double dp_ = (p_ - _p) / h;
        double volga = (p_ - 2 * p + _p) / (h * h);
        double error_bound = volga * h / 2;

        ensure(fabs(dp - dp_) <= fabs(error_bound));
    }
});
...
static AddIn xai_black_put_implied(
    ...
    .Arg(XLL_DOUBLE, L"t", L"is the time in years to expiration.", L"0.25")
    .Category(L"XLL")
    .FunctionHelp(L"Return the Black implied volatility.")
);
